...
```bash
export KRALAPI_BASE_URL=https://kralapi.kralai.tech
export KRALAPI_MODEL=gpt-5.5
export KRALAPI_API_KEY=你的接口密钥
```
...
source .env
python app.py \
  --model-id /root/autodl-tmp/models/VoxCPM2 \
  --host 0.0.0.0 \
  --port 8808 \
  --device cuda
```
...
点击“改写并生成语音”后，页面会先显示“已改写的方言文本”，然后生成音频。
